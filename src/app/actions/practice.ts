...
function generatePracticeBoard(settings: PracticeSettings): PracticeBoard {
  ...
  return { heroHand, villainHands, opponentCount, board };
}
...
export async function generatePracticeHand(
  ...
  let boardData: PracticeBoard;
  let equity = 0;

  // We don't generate situations where the user is well ahead.
  // This prevents "too easy" call decisions.
  do {
    boardData = generatePracticeBoard(settings);
    const { winPercentage, tiePercentage } = calculateHandStrength(
      boardData.heroHand,
      boardData.board,
      boardData.opponentCount,
      5000,
    );
